...
import { summarizeMeetingHandler } from '../../../src/jobs/handlers/summarize-meeting.js';
...
describe('summarizeMeetingHandler', () => {
  ...
  function makeJob(): Job {
    return {
      id: 'job-summarize-meeting', type: 'summarize-meeting', status: 'running', priority: 50,
  ...
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'karpathy-summarize-meeting-'));
    vault = createFsAdapter(dir);
    rawPath = 'raw/meeting.txt';
    await vault.write(rawPath, 'Tom: Let us discuss the roadmap.');
    summaryPath = 'sources/meeting-summary.md';
    await vault.write(
      summaryPath,
      serializeNote(
        {
          id: 'src1', type: 'source_summary', title: 'Meeting', created_at: '2026-07-01T00:00:00Z',
  ...
  it('rejects with the original TransientLLMError when meeting summarization fails transiently', async () => {
    const llm: LLMClient = {
    ...
    await expect(summarizeMeetingHandler.execute(makeJob(), ctx)).rejects.toBeInstanceOf(TransientLLMError);
  });
  ...
  it('rejects with a plain Error (not TransientLLMError) on a non-transient failure', async () => {
    ...
    let caught: unknown;
    try {
      await summarizeMeetingHandler.execute(makeJob(), ctx);
    } catch (err) {
      caught = err;
    }
    expect(caught).toBeInstanceOf(Error);
    expect(caught).not.toBeInstanceOf(TransientLLMError);
    expect((caught as Error).message).toContain('Meeting summarization failed');

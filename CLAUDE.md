...
- **Concept glossary** (`src/maintenance/concept-glossary.ts`) — concepts are no longer individual wiki pages. `compile-entities.ts` upserts every mention into `{layout.wiki}/concepts/glossary.md` via `upsertConceptMention` — one section per concept, appending sources instead of creating pages. `migrate-concept-glossary` is a one-time job that consolidates pre-existing concept pages into it.

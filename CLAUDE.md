...
```jsonc
{
  ...
  "daemon": {
    "host": "127.0.0.1",
    "port": 8765,
    "tickIntervalMs": 300000,        // scheduler cadence; the daemon fire-and-forget spawns an isolated `intel tick` child on this interval, replacing the standalone com.karpathy.tick launchd job
    ...
    "watchdogEnabled": true,               // event-loop watchdog (heartbeat worker thread SIGKILLs a wedged daemon for launchd to restart)
    "watchdogTimeoutMs": 30000,             // max ms the heartbeat may go stale before the watchdog acts
    "watchdogHeartbeatMs": 1000,            // main-thread heartbeat stamp cadence
    "schedulerChildMaxRuntimeMs": 600000     // max ms a scheduler-tick child may run before it's treated as stuck (killed + respawned)
  },
  "ingest": {
...
- **Entry point** — `runDaemon` (`src/mcp/daemon.ts`) builds one `MCPContext`, acquires a single-instance advisory lock (`createFileLock`, key `'mcp-daemon'`; a second instance for the same project root exits cleanly with a no-op handle rather than erroring), then starts the HTTP transport, the single vault watcher (`startVaultWatcher`, reusing §27.1's `acquireWatcherLock`), the isolated scheduler-child interval, and (when enabled) the event-loop watchdog — all sharing that one `ctx`. `applySelfLowPriority(5)` (`src/shared/low-priority.ts`, wraps `os.setPriority`) is applied defensively at boot. Shutdown (`SIGTERM`/`SIGINT`, or a failed startup) runs through one idempotent, exception-safe `teardown()` (`runTeardownSteps`, `Promise.allSettled`-based) so the daemon/watcher locks are always released even if one cleanup step throws; the watchdog is stopped FIRST in that step list (before `http.close()`/watcher/lock) so it never mistakes teardown's own I/O for a hang.
...
- **Scheduler, isolated (daemon event-loop isolation)** — the daemon's own `setInterval` (cadence `daemon.tickIntervalMs`, default 300000ms, `unref()`'d) no longer runs `runSchedulerTick` inline; it calls `createSchedulerChildRunner(...).tick()` (`src/mcp/scheduler-child.ts`), which spawns `karpathy intel tick --project-root <projectRoot>` as a low-priority, detached, unref'd CHILD PROCESS (`spawnLowPriority`) and returns immediately — a slow or wedged job handler inside a tick can no longer block `/mcp`/`/health` for every connected window, because it never runs on the daemon's own event loop at all. The runner itself guards overlap (skips `tick()` while the previous child's runtime is still within `daemon.schedulerChildMaxRuntimeMs`) and kills+respawns a runaway child past that budget; a `child.on('error', ...)` handler (e.g. a spawn-level `ENOENT`) logs and clears tracking instead of throwing uncaught. `intel tick` (`src/bin/intel-command.ts`) itself now honors `--project-root` (parsed via the same `parseProjectRootArg` helper `mcp-daemon`/`daemon install`/`daemon status` use) so the isolated child resolves the DAEMON's project, not `process.cwd()`. `runSchedulerTick` (`src/intelligence/scheduler-tick.ts`) itself is unchanged: backfill/Cursor-import/`tickScheduler`/`runner.runAll()`, power-gating `HEAVY_SCHEDULED_JOBS` (`decay-scan`, `digest-weekly`, `research-propose`, `rot-scan`) off battery/thermal state via a `pmset`-based `powerState()` probe (2s-timeout-bounded, defaults unconstrained on any failure) — light/deterministic jobs and interactive tool calls are never gated. Design: `docs/superpowers/specs/2026-08-10-daemon-event-loop-isolation-design.md`.
- **Watchdog** — `startWatchdog` (`src/mcp/watchdog.ts`), gated on `daemon.watchdogEnabled` (default `true`): the main thread stamps a `SharedArrayBuffer`-backed heartbeat every `daemon.watchdogHeartbeatMs` (default 1000ms, `unref()`'d — a main-process timer, kept alive by the HTTP listening socket regardless); an independent worker thread (`src/mcp/watchdog-worker.ts`, its own tsup entry → `dist/mcp/watchdog-worker.js`) polls that heartbeat on the same cadence and, once it's stale past `daemon.watchdogTimeoutMs` (default 30000ms), `process.kill(process.pid, 'SIGKILL')`s the whole process (worker threads share their owning process's PID) for launchd's `KeepAlive` to restart. The worker's own interval is deliberately left ref'd (NOT `.unref()`'d, unlike every other timer in this codebase) — it's the only handle keeping that thread's event loop alive, so unref'ing it made the worker exit immediately (code 0) before ever checking a heartbeat, which fed `startWatchdog`'s "respawn on unexpected exit" logic into a tight, un-throttled respawn loop; caught live via this task's manual daemon smoke test, fixed, and covered by a real (non-mocked) `worker_threads.Worker` regression test. `runDaemon`'s own `workerPath` is computed as `fileURLToPath(new URL('../mcp/watchdog-worker.js', import.meta.url))`, NOT `'./watchdog-worker.js'` — `daemon.ts` has no standalone tsup entry and gets inlined into `dist/bin/karpathy.js`, so `import.meta.url` resolves to that bundle's own location, and only the `'../mcp/...'` form resolves correctly against the real build in both that layout and a hypothetical future standalone one.

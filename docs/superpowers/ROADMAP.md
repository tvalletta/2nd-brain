...
| 2 — Pool + judge + calibrate | ✅ **DONE** (2026-07-07, real full-pool run) | Triage proposals (`eval/dataset/triage-proposals.json`); pooled ground truth (`eval/dataset/pool.json`); dual-judge + behavioral-shortcut grading (`eval/pool/judge.ts`, `judge-full.ts`, `behavioral-shortcut.ts`) run for real against the live production index: **2,075 judgments across 69 of 73 items** (4 skipped, see I13) — `label_provenance`: 1,997 `llm` (dual-judge reconciled) / 78 `behavioral` (shortcut fired); 2 disagreements (0.1%). Supersedes the retired 20-item human-calibration sample; the human calibration gate itself is retired per `2026-07-07-eval-judging-v2-design.md` §3. `eval/results/2026-07-07-calibration-sample.md` remains only as a historical record of Tom's initial (abandoned) review attempt. |
...
| I13 | Real full-pool `eval:judge-full` run (Task 7, 2026-07-07) skipped 4 of 73 items, two distinct failure modes, both caught and logged rather than crashing: (a) **new bug** — `plaud-ai-session-018` (56 candidates), `hot-topics-005` (53 candidates), `decisions-017` (49 candidates) failed Zod validation with `invalid_type: expected "array", received "object"` from `JudgeResponseSchema` (`eval/pool/judge.ts`) — the judge model returned a well-formed JSON object (likely wrapping the results in a top-level key, e.g. `{"judgments":[...]}`) instead of a bare array on these particular (larger-candidate-count) calls; distinct from I10's escaping bug, since the response parsed as valid JSON, just the wrong shape. (b) **transient infra** — `plaud-ai-session-014` (46 candidates) failed with a Bedrock 500 (`"The system encountered an unexpected error during processing. Try your request again."`) — a one-off service error, not a code bug; not retried in this run. All 4 were caught per-item and skipped (not crashing the run), consistent with the "small number acceptable" bar in `task-7-brief.md` Step 4 (well under the 5-10 threshold). Not fixed here — flagged for Track B follow-up. | `pnpm eval:judge-full` console output, 2026-07-07; `task-7-report.md` | 🟡 | needs a real fix: (a) either tighten the judge prompt to be explicit about top-level-array-only output, or make `extractStructured`/the judge call unwrap a single-key object wrapper before Zod validation as a forgiving fallback; (b) consider a retry-once-on-5xx wrapper around `extractStructured` for transient Bedrock errors before treating a call as failed. |
...
20-item calibration-sample file**: **2,075 judgments across 69 of 73 items**
(4 items skipped — see new issue I13 below for the two distinct failure
modes hit; neither is a recurrence of I10). Provenance: 1,997 `llm`
(dual-judge reconciled) + 78 `behavioral` (shortcut fired, proving it works
end to end). Disagreements: 2 out of 2,075 (0.1%) — written to
`eval/results/2026-07-07-disagreements.md`, a small fraction well under the
20% flag-worthy threshold. I9 (author-absent, 0/10 candidates —
`queries.json` has zero `absent`-subtype items) remains open and is out of
scope for this plan. **Next, still unbuilt:** (1) Phase 3 scoring/scorecard,
now unblocked since `judgments.json` is complete
(`eval/results/*-runs.json` + `judgments.json` → recall/precision/MRR per
spec §7); (2) I13 (the two new failure modes from this run); (3) I9
(absent-query scoring); (4) document the passive-telemetry refresh cadence
per the design's §6 (re-run `eval:mine`'s behavioral-signal extraction
before any future full-pool judging run, to pick up new usage since the last
run). See task-7-brief.md "Notes for the next plan" for the fuller
breakdown.

...
**Status:** Approved for plan write-up (design conversation complete 2026-07-31, run in minimal-interaction mode per operator instruction; §16's nickname-scope question decided below, the `reviewEnabled` item remains an explicit operator recommendation — a real config change to Tom's live vault, not something this session changes unilaterally. Neither blocks plan write-up.)
...
**Correction (whole-branch review, post-implementation):** as originally shipped, `extract-entities.ts`'s handle→name attachment (`handleIdMap.has(p.name.toLowerCase())`) only matched when the LLM's extracted person name was the *literal handle string itself* — e.g. the extractor returning `"pino"` verbatim. It did **not** match a fuller "First Last" extracted name that merely *contains* the handle as a token (e.g. extracted name `"Bryan Pino"` against handle key `"pino"`), so the "Frank Brown" example above did not actually hold even for same-document mentions unless the LLM extracted the bare handle string. The whole-branch review closed this specific gap with a narrow, safe extension (`resolveExternalIdsForName` in `extract-entities.ts`): after the exact-match check, it also checks whether the handle equals a whole, case-insensitive **token** of the extracted name (not a raw substring — `"ed"` inside `"Edward Norton"` deliberately does not match, only a full token like `"pino"` inside `"Bryan Pino"`). This closes the real, evidenced "Bryan"/"Bryan Pino" pattern *when both the handle link and the fuller name appear in the same raw document*. It does **not** close the "Frank Brown"/`brownf`-style surname+initial abbreviation pattern (no token of "Frank Brown" equals "brownf"), nor the cross-document case with no shared raw text at all — both remain accepted residual limitations; see §12.
...
- **Name→handle attachment scope (whole-branch review finding, partially closed):** the initial implementation only attached `external_ids` when the LLM's extracted person name was the *literal handle string* (`handleIdMap.has(p.name.toLowerCase())`), so even the same-document case — a fuller "First Last" name and a `[@handle](...)` link both present in one raw source — did not resolve via Tier 0 unless the LLM extracted the bare handle verbatim. The whole-branch review added a narrow, safe token-equality extension (`resolveExternalIdsForName`, `src/jobs/handlers/extract-entities.ts`): after the exact-match check, the handle is also matched against each whitespace-delimited **token** of the extracted name (case-insensitive, exact token equality — not substring containment, to avoid false positives like handle `"ed"` matching inside `"Edward Norton"`). This closes the real "Bryan"/"Bryan Pino" pattern when both the handle link and the fuller name co-occur in the same document. It deliberately does **not** attempt the "Frank Brown"/`brownf`-style surname+initial abbreviation match (no exact token of "Frank Brown" equals "brownf") — extending to that would require materially fuzzier matching (partial substrings, initials reconstruction) with meaningfully higher false-positive risk, and remains an accepted residual gap alongside the item above. The cross-document case (handle in one document, fuller name in an entirely separate document with no shared raw text) is unaffected by this extension either way — `handleIdMap` is always scoped to a single document's raw text — and continues to rely on Components 2/3's reconciliation-queue detection instead.
...
- **Nickname-table scope: decided, keep as designed.** The built-in seed list stays small and Anglophone (§3), with `enrichment.personResolution.extraNicknameGroups` as the config-driven extension point — not expanded to cover Armenian names or other locales in this spec. Reasoning: the design's own stated principle ("false equivalences are worse than missed ones") applies directly here — guessing at nickname/spelling-variant equivalence classes for names without confident native-speaker knowledge risks introducing wrong equivalences that silently misresolve two different people, which is a worse failure mode than the status quo (no equivalence, handled instead by Component 2/3's queue-bound detection once a fuller name eventually appears). Tom can add locale-specific groups via config as real cases surface in his own vault.
- **`maintenance.reviewEnabled` remains an explicit operator recommendation, not something this spec (or this session) changes unilaterally.** It's currently `false` in the real vault's live config at `~/.karpathy/config.json` — a production setting outside this git repo, not a code change this design or its implementation touches. Component 3 (immediate ingest-time detection) works regardless of this flag; only the periodic `detect-entity-dupes` cron sweep (Component 2's scheduled half) depends on it. Recommendation stands: flip `reviewEnabled: true` to get the full benefit of this spec once it ships — surfaced directly to Tom, left for him to action.

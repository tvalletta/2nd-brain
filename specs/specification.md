...
### 27.11 Bounded embedding-table scans (Fix C)

A third root-cause audit, focused on the ~2GB/250-400k-row `embeddings.sqlite` table (`better-sqlite3`), found `EmbeddingStore.all(filter)` (`src/embeddings/store.ts`) ran `SELECT ... FROM embeddings WHERE provider_id = ?` with no date or `LIMIT` predicate, `.all()`-materializing every row — Float32Array vector, parsed metadata object, and full chunk text — before applying any JS-side filter. Two schedulers hit this on every run: `src/intelligence/digest.ts` (weekly) filtered to `updated_at >= cutoff` in JS after full materialization; `src/intelligence/research-propose.ts` (daily) called `store.all()` with no filter at all, and its own `mentionStats()` helper already discarded every row older than a 14-day `recentCutoff` — meaning the daily job fully materialized the entire multi-hundred-thousand-row table purely to throw most of it away.

Fix: `EmbeddingStore` gained `allSince(cutoffIso: string): EmbeddingRow[]`, which pushes an `updated_at >= ?` predicate into SQL (backed by a new composite index, `idx_emb_provider_updated ON embeddings(provider_id, updated_at)`) instead of filtering in JS after the fact. `updated_at` is written by `upsert()` as `new Date().toISOString()` — a fixed-width, UTC (`Z`-suffixed) ISO-8601 string — so plain lexicographic string comparison in SQL is equivalent to numeric timestamp comparison. `digest.ts` now calls `store.allSince(cutoffIso)` (the same `cutoffIso` it already computed for the digest window) instead of `store.all(dateFilter)`; `research-propose.ts` now calls `deps.store.allSince(recentCutoffIso)` (derived from the same `recentCutoff` `mentionStats()` already enforced) instead of `deps.store.all()`. Neither caller's own filtering/scoring logic changed — only the row set that reaches it shrinks from all-time to the relevant window, so results are unchanged while peak materialized memory is bounded to the window's row count rather than the whole table. The existing unfiltered `all()`/`all(filter)` method is unchanged for any other caller.

### 27.12 Batched FTS bulk body reads (Fix D)

`src/search/fts-index.ts`'s `sync()` pre-read the full body of every changed file into a single in-memory `mutations` array before opening the write transaction (a `better-sqlite3` constraint: transactions must run synchronously, so async file reads can't happen mid-transaction). On a first `--populate-fts` run, or after a bulk mtime-touching resync (e.g. a OneDrive-driven mass touch of the vault), that array held all ~22k changed file bodies at once — hundreds of MB.

Fix: `search.ftsSyncBatchSize` (config, default 500). `sync()` still computes the on-disk/indexed mtime baseline and the changed-vs-unchanged/deleted split up front (cheap — paths and mtimes only, not bodies), but now processes upserts in batches: read one `batchSize`-sized batch of file bodies, commit that batch in its own transaction, release, repeat. Deletes (which need no body read) are applied in a single up-front transaction. `FTSIndexOptions` gained an optional `batchSize` field; `openHybridStoreFromConfig` (`src/search/factory.ts`) threads `config.search.ftsSyncBatchSize` through. Peak memory during a full sync is now bounded to `batchSize` file bodies instead of the entire changed set; final index state (added/updated/removed/unchanged counts, and the indexed content itself) is unchanged.

### 27.13 Safe embedding chunk sizes (Fix K)

The daemon log showed Ollama returning HTTP 500 ("input length exceeds the context length"). `src/jobs/handlers/embedding-index.ts` called `chunkText(body, 1200, 4000)` — the 4000-char hard cap assumed ~2.5 chars/token, but `store.ts`'s own chunking-helper docstring documents wikilink-dense markdown tokenizing as low as ~1.5 chars/token, meaning a 4000-char chunk (≈2667 tokens at that density) could exceed nomic-embed-text's 2048-token cap. Separately, `src/agent/skills/embedding-match.ts`'s `matchSkillByEmbedding` embedded its `content` argument with zero chunking or truncation at all.

Fix: `embeddings.maxChunkChars` (config, default 2048 — safe even at 1.5 chars/token, with margin to spare). `embedding-index.ts` now calls `chunkText(body, 1200, ctx.config.embeddings.maxChunkChars)` instead of the hardcoded 4000. `matchSkillByEmbedding` gained an optional 4th parameter, `maxChunkChars` (defaulting to the same 2048 constant), and truncates both the query `content` and every skill's composed text (`name` + `description` + `patterns`) before calling `provider.embed(...)`; its sole call site (`src/agent/runner.ts`) threads `jobContext.config.embeddings.maxChunkChars` through. As a defensive backstop independent of any caller, the Ollama provider (`src/embeddings/ollama.ts`) gained an optional `maxInputChars` option (same 2048 default, wired from `embeddings.maxChunkChars` in `createProviderFromConfig`); `postEmbedding()` now truncates any input longer than this cap before building the POST body, logging a warning (`Truncating oversized embedding input before POST`) when it does — so no oversized input, regardless of caller, can 500 the provider again.

### 27.14 Constraints (§27.11-27.13)

- `allSince(cutoffIso)` MUST return the same set of rows `all(filter)` would return under an equivalent `updated_at`-based JS filter — the SQL predicate is a performance change, not a semantic one.
- `cutoffIso` arguments to `allSince` MUST use the same ISO-8601 UTC (`Z`-suffixed) format `upsert()` writes — a mismatched format (e.g. missing milliseconds, non-UTC offset) would break the lexicographic-comparison assumption.
- `sync()`'s batching MUST NOT change final index state — the set of indexed docs, and each doc's title/body, after a full sync must be identical regardless of `batchSize`; only peak memory and the number of transactions differ.
- Deletes in `sync()` MUST NOT depend on file body content — they're computed and applied from paths alone before any batch's body read.
...
- an internal `setInterval` that calls `createSchedulerChildRunner(...).tick()` (`src/mcp/scheduler-child.ts`) on a `daemon.tickIntervalMs` cadence (default 300000ms / 5 min), replacing the standalone `com.karpathy.tick` launchd job for this project — see §28.3 for why this is a child PROCESS rather than an inline call, as it was before daemon event-loop isolation (§28.3.1);
- when `daemon.watchdogEnabled` (default `true`), an event-loop watchdog (`startWatchdog`, `src/mcp/watchdog.ts`) — see §28.3.2.
...
`runSchedulerTick` (`src/intelligence/scheduler-tick.ts`, extracted from the CLI's former inline `intel tick` case, which now delegates to it) runs first-run backfill, Cursor session import, `tickScheduler` (fires whatever's due from `defaultSchedule()`), and drains the queue via `runner.runAll()` — the latter already guarded by §27.6's global `'queue-runner'` lock, so an isolated scheduler-child tick (§28.3.1) and a manual `karpathy intel tick` can never double-drain.
...
#### 28.3.1 Daemon event-loop isolation: the scheduler runs as a child process, not inline

A follow-up audit found that running `runSchedulerTick` inline on the daemon's own `setInterval` callback (the original §28 design, guarded only by a same-process `tickInFlight` re-entrancy flag) meant any slow or wedged job handler inside a tick — an LLM call that hangs, a pathological vault scan — blocked the SAME event loop serving every connected window's `/mcp`/`/health` requests, defeating the "one shared daemon" design's core promise of staying responsive. Full design: `docs/superpowers/specs/2026-08-10-daemon-event-loop-isolation-design.md`.

- **`createSchedulerChildRunner`** (`src/mcp/scheduler-child.ts`) replaces the inline call. `tick()` is synchronous and non-blocking: fire-and-forget, it never awaits the child. It spawns `karpathy intel tick --project-root <projectRoot>` via `spawnLowPriority` (§28.4's NFR-3 low-priority wrapper) with `detached: true`, `stdio: 'ignore'`, `cwd: projectRoot`, then immediately `child.unref()`s it — the child's own lifetime is independent of the daemon process and is NOT tracked through daemon shutdown (the detached child is intentionally left running to completion; it is not killed on `teardown()`).
- **Overlap guard** — `tick()` no-ops (logs and returns) while the previously-spawned child's wall-clock runtime is still within `daemon.schedulerChildMaxRuntimeMs` (default 600000ms / 10 min) and the child hasn't exited. Past that budget, the runner treats the child as stuck: `SIGKILL`s it and spawns a fresh one on that same `tick()` call.
- **Failure isolation** — both `child.on('exit', ...)` (clears tracking) and `child.on('error', ...)` (e.g. a spawn-level `ENOENT`; logs and clears tracking) are handled — an unhandled `'error'` event would otherwise throw uncaught and crash the daemon, exactly the failure class this whole mechanism exists to prevent.
- **`intel tick --project-root`** — `src/bin/intel-command.ts`'s `tick` subcommand parses `--project-root` (via the same `parseProjectRootArg` helper `mcp-daemon`/`daemon install`/`daemon status` use) and threads it into `loadConfig(projectRoot)`, so the isolated child resolves the DAEMON's project/state, not an ambient `process.cwd()`. (The child's `cwd` is also set to `projectRoot`, so `process.cwd()` would usually have been right too — but this must not be left implicit.)
- **Cadence unchanged** — the daemon's own `setInterval` (calling `scheduler.tick()` instead of `runSchedulerTick(...)` directly) is still keyed to `daemon.tickIntervalMs` and still `unref()`'d, since the HTTP listening socket, not this timer, is what keeps the daemon process alive.

#### 28.3.2 Event-loop watchdog

Isolating the scheduler (§28.3.1) removes one source of a wedged daemon event loop, but not all of them (a pathological interactive tool call on the HTTP transport itself, for instance). `startWatchdog` (`src/mcp/watchdog.ts`), gated on `daemon.watchdogEnabled` (default `true`), detects and recovers from a wedged main event loop independently of anything running on it:

- The main thread stamps a `SharedArrayBuffer`-backed heartbeat (`Atomics.store`) every `daemon.watchdogHeartbeatMs` (default 1000ms) via its own `unref()`'d `setInterval` — an initial beat is written BEFORE the worker below is spawned, so there is no false trip at `t=0`.
- An independent worker thread (`src/mcp/watchdog-worker.ts`, its own `tsup` build entry → `dist/mcp/watchdog-worker.js`) polls that same heartbeat on the same cadence and, once `nowMs - lastBeatMs > daemon.watchdogTimeoutMs` (default 30000ms; the pure decision is `shouldTrip`, unit-tested directly), calls `process.kill(process.pid, 'SIGKILL')` — worker threads share their owning process's PID, so this SIGKILLs the whole daemon process for launchd's `KeepAlive` to restart. The worker's own heartbeat-check interval is deliberately left ref'd (not `.unref()`'d) — unlike every other timer in this codebase, it is the ONLY handle keeping that worker thread's event loop alive, so unref'ing it causes the worker to exit immediately after spawning, before ever checking a heartbeat.
- `startWatchdog` respawns the worker if it exits unexpectedly (`worker.on('exit', ...)`, guarded by a `stopping` latch so a deliberate `stop()` never triggers a respawn) and logs (never throws) on `worker.on('error', ...)`.
- `WatchdogHandle.stop()` clears the main-thread heartbeat interval and calls `worker.terminate()` (a hard stop that ignores refs). In `runDaemon`'s teardown, this is the FIRST step in `runTeardownSteps([...])` — it must stop observing before the rest of teardown starts tearing down the very process it's watching, so a slow `http.close()`/watcher-stop during a clean shutdown can never be mistaken for a hang.
- `runDaemon` resolves `startWatchdog`'s `workerPath` as `fileURLToPath(new URL('../mcp/watchdog-worker.js', import.meta.url))` — deliberately NOT `'./watchdog-worker.js'`. `daemon.ts` has no standalone `tsup` build entry (see §28.1) and is inlined into the single `dist/bin/karpathy.js` bundle, so `import.meta.url` there resolves to that bundle's own location (`dist/bin/`), not to a hypothetical `dist/mcp/daemon.js`; only the `'../mcp/...'` form resolves to the real `dist/mcp/watchdog-worker.js` in both this bundled layout and a hypothetical future standalone-entry layout.

Together, §28.3.1's child-process isolation and this watchdog are what make retiring the standalone `com.karpathy.tick` launchd job (§28.1) safe on an ongoing basis, not just at cutover: that job was an independent, launchd-driven timer that fired every 5 minutes regardless of any one process's health, so replacing it with the daemon's own in-process `setInterval` made the daemon a single point of failure for scheduling. Without both mechanisms, a wedged daemon event loop would silently stop firing scheduled ticks forever, with nothing else left to drive `runSchedulerTick`; with them, a slow tick can no longer wedge the loop in the first place, and a loop wedged for any other reason (e.g. a pathological tool call) is SIGKILLed and restarted by launchd's `KeepAlive`, so scheduling always resumes on its own.
...
- **Sleep-aware (NFR-2)** — the launchd plist (`renderDaemonPlist`, `src/mcp/daemon-plist.ts`) deliberately omits `StartInterval`/`StartCalendarInterval`/`WakeInterval`, using `KeepAlive` instead: no periodic wake-and-poll cycle, no power assertion, no `caffeinate`. `setInterval` is paused by the OS during sleep and fires once on wake; `createSchedulerChildRunner`'s overlap guard (§28.3.1) means that one post-wake fire can never stack a new child on top of one still within its runtime budget.
- **Low-priority (NFR-3)** — the plist sets `ProcessType: Adaptive` (low priority idle, responsive while serving), `LowPriorityIO: true`, `Nice: 5`; `runDaemon` additionally calls `applySelfLowPriority(5)` (`src/shared/low-priority.ts`, wrapping `os.setPriority`) defensively at boot, since plist QoS settings are launchd-enforced only when the process is actually launched by launchd. Any process the daemon spawns (the scheduler-child's `intel tick`, §28.3.1; the Stop-hook background drain) is wrapped in `taskpolicy -b` when available (checked at both `/usr/bin/taskpolicy` and `/usr/sbin/taskpolicy`), falling back to a bare spawn + `os.setPriority` on platforms without it.
...
Once the lock is held, `SIGTERM`/`SIGINT` handlers are registered *before* startup begins, so a signal arriving mid-startup still routes to the same `teardown()` closure a clean shutdown or a startup failure would use. `teardown()` is idempotent and exception-safe: every cleanup step (stop the watchdog — FIRST, §28.3.2; close the HTTP server; stop the watcher; release the watcher lock; release the daemon lock) runs via `runTeardownSteps`, which uses `Promise.allSettled` so one step throwing can never prevent the others from running — the daemon lock is always released, even on a partial failure. The detached scheduler-child process (§28.3.1) is intentionally NOT tracked or killed by `teardown()` — it runs to completion independently of the daemon's own lifecycle.
...
- The daemon's own `setInterval` callback (`scheduler.tick()`, §28.3.1) MUST NOT block the daemon's event loop or call `process.exit` on the daemon process — it is synchronous and fire-and-forget, spawning `intel tick` as a separate, detached child process. That CHILD process's own `intel tick` invocation calling `process.exit` afterward is expected and does not affect the daemon.
...
- `teardown()` MUST be idempotent and MUST run every cleanup step even when an earlier step throws — no code path may leak the daemon or watcher lock past process exit. The watchdog (§28.3.2) MUST be stopped before the other teardown steps run.
- Any timer intended to keep a thread/process alive for its own sake (the watchdog worker's heartbeat-check interval, §28.3.2) MUST NOT be `.unref()`'d — that pattern is correct only for timers on the daemon's main process, where the HTTP listening socket is the thing actually keeping it alive.
